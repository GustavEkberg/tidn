...
// 1. INPUT SCHEMA
// ============================================================
const EditMediaCommentInput = S.Struct({
  id: S.String.pipe(S.minLength(1)),
  text: S.String.pipe(S.minLength(1), S.maxLength(2000))
});

type EditMediaCommentInput = S.Schema.Type<typeof EditMediaCommentInput>;

// ============================================================
// 2. ACTION FUNCTION
// ============================================================
export const editMediaCommentAction = async (input: EditMediaCommentInput) => {
  return await NextEffect.runPromise(
    Effect.gen(function* () {
      // --------------------------------------------------------
      // 3. VALIDATE INPUT
      // --------------------------------------------------------
      const parsed = yield* S.decodeUnknown(EditMediaCommentInput)(input).pipe(
        Effect.mapError(
          () =>
            new ValidationError({
      ...
      // 10. TRACING
      // --------------------------------------------------------
      Effect.withSpan('action.comment.editMediaComment', {
        attributes: { operation: 'comment.editMediaComment' }
      }),
      ...
      // 12. LOG ERRORS
      // --------------------------------------------------------
      Effect.tapError(e => Effect.logError('action.comment.editMediaComment failed', { error: e })),
